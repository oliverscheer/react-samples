...
const AudioBars = (props: AudioBarsProps) => {
    ...
    const dataMap = new WeakMap<Float32Array, { [key: string]: { [key: string]: number[] } }>();
    ...
    const normalizeArray = (
        ...
        const result: number[] = new Array<number>(arrayLength);
    ...
    const WavRenderer = {
        drawBars: (data: Float32Array) => {
            ...
            // const pointCount = 20
            const points = normalizeArray(data, pointCount, true) as number[];
            ...
            for (let i: number = 0; i < pointCount; i++) {
                const amplitude = 1 - Math.abs(1 - points[i] * 2);
